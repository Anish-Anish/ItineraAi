...
# -------------------------
PER_KM_COST = 15
MAX_TRAVEL_DISTANCE_PER_SPOT = 200
MAX_DAILY_TRAVEL_MIN = 480

load_dotenv()

OPENWEATHER_API_KEY = os.getenv("OPENWEATHER_API_KEY")
OUTPUT_FILE = "fin_planner/fast_output.json"
SERVICE_ACCOUNT_PATH = os.getenv("SERVICE_ACCOUNT_PATH")
VERTEX_PROJECT = os.getenv("VERTEX_PROJECT")
VERTEX_LOCATION = os.getenv("VERTEX_LOCATION")
MODEL_ID = os.getenv("MODEL_ID_PLANNER")
SCOPES = [os.getenv("SCOPES")] if os.getenv("SCOPES") else ["https://www.googleapis.com/auth/cloud-platform"]
...
def get_structured_trip_details(user_prompt: str) -> Optional[TripDetails]:
    print(f" Checking service account path: {SERVICE_ACCOUNT_PATH}")
    ...
    try:
        ...
        print(f"Initializing Vertex AI client for project: {VERTEX_PROJECT}")
        client = genai.Client(
            vertexai=True,
...
def test_vertex_ai_setup():
    print(f" Testing Vertex AI Setup:")
    print(f"  SERVICE_ACCOUNT_PATH: {SERVICE_ACCOUNT_PATH}")
    print(f"  VERTEX_PROJECT: {VERTEX_PROJECT}")
    print(f"  VERTEX_LOCATION: {VERTEX_LOCATION}")
    print(f"  MODEL_ID: {MODEL_ID}")
    print(f"  SCOPES: {SCOPES}")

    if not SERVICE_ACCOUNT_PATH:
        print("  SERVICE_ACCOUNT_PATH is None")
        return False
...
def fix_broken_json(bad_json: str) -> dict:
    print(" Attempting to auto-fix malformed JSON...")

    # First try to clean markdown blocks if present
...
def format_itinerary_with_llm(itinerary_data, user_query, plan):
    ...
    prompt = f"""
        ...
        1️ Group nearby spots on the same day to minimize travel.
        2⃣ Start each day near the hotel and pick user-requested or nearby places.
        3️ Allocate realistic durations (1–2h for small spots, 3–5h for beaches, etc).
        4⃣ Each plan must be a **valid JSON object** matching the schema below.
        5⃣ Output an array containing only one plane — `[plan1]`.
    ...
    try:
        ...
        print(" JSON parsing failed, trying auto-fix")
        cleaned_output = clean_json_output(refined_output)
